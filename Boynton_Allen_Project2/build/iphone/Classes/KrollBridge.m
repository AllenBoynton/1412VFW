...
NSString * Boynton_Allen_Project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation Boynton_Allen_Project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_boynton_allen_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Boynton_Allen_Project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Boynton_Allen_Project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_boynton_allen_project2 = [[Boynton_Allen_Project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_boynton_allen_project2];
	
	NSString *_boynton_allen_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _boynton_allen_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_boynton_allen_project2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Boynton_Allen_Project2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.boynton_allen_project2.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.boynton_allen_project2.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
